...
import RemoveButton from "./[id]/_components/remove-button";

const ProductList = ({ products }: { products: Product[] }) => {
  const [optimisticProducts, removeOptimisticProduct] = useOptimistic(
    products,
    (currentProducts, optimisticId) => {
      return currentProducts.filter(
        (product: Product) => product.id !== optimisticId
      );
    }
  );
            ...
            {optimisticProducts.map((product: Product) => (
              ...
              >
                <div className="flex flex-row justify-between">
                  ...
                  <RemoveButton
                    id={product.id}
                    optimisticAction={removeOptimisticProduct}
                  />
                </div>
